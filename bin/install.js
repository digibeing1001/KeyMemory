...
if (IS_WIN) {
  ...
  if (pathStr.toLowerCase().split(';').some(p => p.toLowerCase() === BIN_DIR.toLowerCase())) {
    console.log('  \x1b[32m✓ keymemory-ui 命令已注册\x1b[0m');
  } else {
    try {
      execSync(
      ...
      console.log('  \x1b[32m✓ keymemory-ui 命令已注册到系统 PATH\x1b[0m');
    } catch {
...
console.log('');
console.log('    \x1b[36mkeymemory-ui\x1b[0m          一键启动 Web UI');
console.log('    \x1b[36mpnpm start:ui\x1b[0m        从项目目录启动');
console.log('');
console.log('  \x1b[2m⚠ 新终端窗口需要重新打开才能使用 keymemory-ui 命令\x1b[0m');
console.log('');
