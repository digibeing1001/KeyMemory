...
function auditManifest() {
  ...
  for (const scriptName of ['typecheck', 'build', 'eval:memory', 'perf:memory', 'smoke', 'smoke:mcp', 'smoke:agent-connect', 'smoke:loop', 'smoke:mailbox', 'smoke:llm', 'smoke:automation', 'smoke:launchers', 'release:check']) {
...
function auditReleaseArtifacts() {
  ...
  assertFile('scripts/smoke-memory-automation.mjs', /(?=[\s\S]*without a local embedding model)(?=[\s\S]*cross agent_space boundaries)(?=[\s\S]*bootstrap a concrete work thread)(?=[\s\S]*false conflict)/, 'relation fallback, mailbox bootstrap, isolation, and conflict precision coverage');
